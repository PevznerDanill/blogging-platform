...
class UserBlogListView(ListView):
    ...
    def get_queryset(self):
        ...
        return Blog.objects.select_related('profile').prefetch_related('posts').filter(profile__pk=profile_pk)
...
class PostCreateView(UserPassesTestMixin, LoginRequiredMixin, CreateView):
    ...
    def test_func(self):
        pk = self.kwargs.get('pk')
        cur_blog = Blog.objects.select_related('profile').get(pk=pk)
        cur_user = Profile.objects.get(user=self.request.user)
...
def publish_or_archive(request: HttpRequest, pk) -> HttpResponseRedirect:
    cur_post = Post.objects.get(pk=pk)
